...
;;; Usage:
;;
;; - $ gtags :: go to the root of your linux, or any C project, directory and type gtags.
...
;;
;; - ggtags-find-tag-dwim :: Find a tag by context.
;;
;;                          If point is at a definition tag, find references,
;;                          and vice versa. If point is at a line that matches
;;                          ggtags-include-pattern, find the include file
;;                          instead.
;;
;;                          To force finding a definition tag, call it with a
;;                          prefix (C-u).
;;
;; - ggtags-find-tag-mouse :: Like ggtags-find-tag-dwim but suitable for binding to mouse events.
;; - ggtags-find-definition :: Find definition tags. With C-u ask for the tag name with completion.
;; - ggtags-find-reference :: Find reference tags. With C-u ask for the tag name with completion.
;; - ggtags-find-other-symbol :: Find tags that have no definitions. With C-u ask for the tag name with completion.
;; - ggtags-find-tag-regexp :: Find definition tags matching a regexp. By default it lists all matching tags in the project. With C-u restrict the lists to a directory of choice.
;; - ggtags-idutils-query :: Use idutils to find matches.
;; - ggtags-grep :: Grep for lines matching a regexp. This is usually the slowest.
